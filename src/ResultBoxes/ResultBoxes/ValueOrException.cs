...
public record ValueOrException<TValue>(TValue? Value, bool IsException) where TValue : notnull
{
    ...
    public TValue GetValue() =>
        (IsException == false ? Value : throw new ResultsInvalidOperationException()) ??
    ...
    public static implicit operator ValueOrException<TValue>(ValueOrException exception) =>
        new(default, true);
}
public record ValueOrException
{
    ...
    public static ValueOrException<TValue> FromValue<TValue>(TValue value) where TValue : notnull =>
        ValueOrException<TValue>.FromValue(value);
}
