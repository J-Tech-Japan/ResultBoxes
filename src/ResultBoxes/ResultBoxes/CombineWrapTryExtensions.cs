...
public static class CombineWrapTryExtensions
{
    public static async Task<ResultBox<TwoValues<TValue1, TValue2>>> CombineWrapTry<
        ...
        => await current.ConveyorResult(
            async first =>
                (await ResultBox<TValue2>.WrapTry(secondValueFunc)).Conveyor(first.Append));
    public static async Task<ResultBox<TwoValues<TValue1, TValue2>>> CombineWrapTry<
        TValue1, TValue2>(
        this ResultBox<TValue1> current,
        ...
        => await current.ConveyorResult(
            async first =>
                (await ResultBox<TValue2>.WrapTry(async () => await secondValueFunc(first.GetValue()))).Conveyor(first.Append));
    ...
    public static ResultBox<TwoValues<TValue, TValue2>>
        ...
        => current.ConveyorResult(
            c => ResultBox<TValue2>.WrapTry(secondValueFunc).Conveyor(current.Append));
    ...
    public static ResultBox<TwoValues<TValue1, TValue2>>
        ...
        where TValue2 : notnull
        => current.ConveyorResult(
            c => ResultBox<TValue2>.WrapTry(() => secondValueFunc(c.GetValue())).Conveyor(current.Append));
