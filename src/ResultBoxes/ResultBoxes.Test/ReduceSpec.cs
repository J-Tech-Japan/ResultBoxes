...
public class ReduceSpec
{
    ...
    public void ReduceWithControlFlow2Break()
    {
        var result = ResultBox.FromValue(Enumerable.Range(1, 10).ToList())
            .ReduceEach(
                0,
                (coming, aggregate) => coming < 5 ? ReduceResultValue.Continue(coming + aggregate)
                    : ReduceResultValue<int>.Break(aggregate));
        Assert.True(result.IsSuccess);
    ...
    public void ReduceWithControlFlow3Error()
    {
        var result = ResultBox.FromValue(Enumerable.Range(1, 10).ToList())
            .ReduceEach(
                0,
                (coming, aggregate) => coming < 5 ? ReduceResultValue.Continue(coming + aggregate)
                    : new ApplicationException("Can not execute"));
    ...
    public void ReduceWithControlFlow4Unit()
    {
        var result = ResultBox.FromValue(Enumerable.Range(1, 10).ToList())
            .ReduceEach(
                UnitValue.Unit,
                (coming, aggregate) => coming < 5 ? ReduceResultValue.Continue(aggregate)
                    : ReduceResultValue.Break(aggregate));
        Assert.True(result.IsSuccess);
    ...
    public void ReduceWithoutControlError()
    {
        var result = ResultBox.FromValue(Enumerable.Range(1, 10).ToList())
            .ReduceEach(
                0,
                (coming, aggregate) => coming < 5 ? coming + aggregate
                    : new ApplicationException("Can not execute"));
        Assert.False(result.IsSuccess);
    }
}
